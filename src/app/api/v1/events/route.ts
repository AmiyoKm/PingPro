...
import { db } from "@/db";
import { DiscordClient } from "@/lib/discord/discord-client";
...
export const POST = async (req: NextRequest) => {
    ...
    if (!user.discordId) {
        return NextResponse.json({ message: "Please enter your discord settings in your account settings" }, { status: 403 })
    ...
    if (quota && quota.count >= quotaLimit) {
        return NextResponse.json({
            message: "Monthly quota reacher. Please upgrade your plan for more events"
        }, {
            status: 429
        }
        )
   ...
   try {
    ...
    const category = user.EventCategories.find((cat) => {
        return cat.name === validationResult.category
    })
    if(!category){
    ...
    const eventData = {
        ...
        description : 
        validationResult.description || `A new ${category.name} event has occurred`,
